...
procedure AlmacenarInformacion (var v: vector; var dimL: rango3);
  ...
  procedure LeerVenta (var v: venta); {carga registro venta}
  begin
    Randomize;{invoca a una funcion especifica de pascal}
    write ('Codigo de producto: ');
    v.codigoP:= random(16); {genera el codigo aleatorio entre 0 y 15}
    writeln (v.codigoP);
    if (v.codigoP <> 0)
    then begin
...
begin
    ...
    begin
       dimL := dimL + 1;
       v[dimL]:= unaVenta; {mete el registro dentro del vector}
       LeerVenta (unaVenta);
    end;
end;
...
begin
     ...
     for i:= 1 to dimL do begin
        if(v[i].codigoP <= 9)then
            write ('0'); {pone esto para que los valores le queden 01,02,...}
        write(v[i].codigoP, ' | ');
     end;
     writeln;
     writeln;
...
procedure Ordenar (var v: vector; dimL: rango3); {algoritmo de seleccion}
...
procedure Eliminar (var v: vector; var dimL: rango3; valorInferior, valorSuperior: rango1);
  ...
  function BuscarPosicion (v: vector; dimL: rango3; elemABuscar: rango1): rango3; {busca posicion en vector de algun codigo}
  var pos: rango3;
  begin
    pos:= 1;
    while (pos <= dimL) and (elemABuscar > v[pos].codigoP) do
       pos:= pos + 1;
    if (pos > dimL) then BuscarPosicion:= 0 {si todos los codigos son menores al codigo buscado}
...
begin
 { Completar }
 {Como se pasa la lista por valor, no es necesario crear variable aux de tipo lista}
 while (L <> nil) do begin
	writeln ('El codigo ', L^.dato.codigoP);
	writeln ('Vendio ', L^.dato.cantidad, ' unidades');
	L:= L^.sig;
	end;
end;
